...
import { checkBreeding, generateRandomHorse } from './logic/breeding.js';
...
export function loadGame() {
  try {
    ...
    if (saved) {
      ...
      window.state.starShards = typeof data.starShards === 'number' ? data.starShards : 0;
...
window.setState = (updates) => {
  ...
  if (['stable', 'title', 'breeding_mare', 'breeding_stallion', 'breeding_confirm', 'race_select', 'hall_of_fame'].includes(window.state.screen)) {
    saveGame();
  }
  render();
};
...
window.handleAction = (action, data) => {
  switch (action) {
    case 'START_RACE':
      ...
      const entryFee = data.fee || 0;
      if (window.state.money < entryFee) {
        alert("出走料が足りません。別のレースを選ぶか、週をスキップして資金を得てください。");
        return;
      }

      // Use forecasted weather if present, otherwise fallback to random
      const weatherOptions = ['Sunny', 'Rainy', 'Muddy'];
      const chosenWeather = data.weather || weatherOptions[Math.floor(Math.random() * weatherOptions.length)];
      const raceWithConditions = {
        ...data,
        weather: chosenWeather,
        trackCondition: data.trackCondition || (chosenWeather === 'Sunny' ? 'Good' : (chosenWeather === 'Rainy' ? 'Heavy' : 'Muddy'))
      };

      const enemies = generateEnemyHorses(11, raceWithConditions.grade);
      const results = simulateRace(horse, enemies, raceWithConditions);
      
      // Update horse career & race history
      const playerRes = results.find(r => r.horseId === horse.id);
      ...
      // Tiered rewards calculation (1st gets 100%, 2nd 40%, 3rd 20%, 4th/5th 8%)
      let earnedPrize = 0;
      let earnedShards = 0;
      ...
      const updatedHorsesForRace = window.state.horses.map(h => {
        if (h.id === horse.id) {
          ...
          const maxStreak = Math.max(h.maxConsecutiveWins || 0, currentStreak);
          return {
            ...h,
            ...
            gradedWins: isGradedWin ? [...(h.gradedWins || []), raceWithConditions.name] : (h.gradedWins || []),
      ...
      window.setState({ 
        ...
        raceResult: results, 
        raceStep: 0,
        money: window.state.money - entryFee + earnedPrize,
        starShards: (window.state.starShards || 0) + earnedShards,
        raceRewards: { prize: earnedPrize, shards: earnedShards },
    ...
    case 'CONFIRM_BREEDING':
      if (window.state.horses.length >= 8) {
        alert("馬房がいっぱいです（最大8頭）。配合を行うには、既存の馬を「引退・売却」して空きを作ってください。");
        return;
      }
      const { selectedMareId, selectedStallionId, selectedMareSource, selectedStallionSource } = window.state;
      let mareObj, stallionObj;

      if (selectedMareSource === 'stable') {
        mareObj = window.state.horses.find(h => h.id === selectedMareId);
      } else {
        mareObj = window.state.mares.find(m => m.id === selectedMareId);
      }

      if (selectedStallionSource === 'stable') {
        stallionObj = window.state.horses.find(h => h.id === selectedStallionId);
      } else {
        stallionObj = STALLIONS.find(s => s.id === selectedStallionId);
      }

      if (!mareObj || !stallionObj) {
        alert("親馬を選択してください。または選択データがリセットされました。最初から選択し直してください。");
        return;
      }
      ...
      if (selectedStallionSource === 'stable') {
        ...
        fee = window.getStallionPrice ? window.getStallionPrice(stallionObj) : (stallionObj.price || stallionObj.fee || 0);
      }
      ...
      const isBlessed = window.state.useStarBlessing && (window.state.starShards >= requiredShards);
      const res = checkBreeding(stallionObj, mareObj, isBlessed);
      
      window.setState({ 
        money: window.state.money - fee,
        starShards: isBlessed ? window.state.starShards - requiredShards : window.state.starShards,
        useStarBlessing: false,
        breedingResult: res,
        screen: 'stable',
    ...
    case 'GENERATE_SCOUT_HORSE':
      if (window.state.horses.length >= 8) {
        alert("馬房がいっぱいです（最大8頭）。新馬をスカウトするには、既存の馬を「引退・売却」して空きを作ってください。");
        return;
      }
    ...
    case 'CONFIRM_SCOUT_HORSE':
      if (window.state.horses.length >= 8) {
        alert("馬房がいっぱいです（最大8頭）。新馬をスカウト・所有するには、既存の馬を「引退・売却」して空きを作ってください。");
        return;
      }
    ...
    case 'CONFIRM_SELL_HORSE':
      ...
      const remainingHorses = window.state.horses.filter(h => h.id !== targetHorse.id);
      ...
      alert(`${targetHorse.name}が引退し、売却資金として¥${(value / 10000).toFixed(0)}万を受け取りました。`);
      break;
    case 'REGISTER_HALL_OF_FAME':
      const horseToRegister = window.state.horses.find(h => h.id === data);
      if (!horseToRegister) return;
      const isAlreadyHOF = (window.state.hallOfFame || []).some(h => h.id === horseToRegister.id);
      if (isAlreadyHOF) {
    ...
    case 'START_TRAINING_GAME':
      {
        ...
        if (currentShards < 3) {
          alert("星片(Star Shards)が足りません！（特訓には3片必要です）\nレースに出走し、上位入賞（1〜3位など）で獲得してきてください。");
          return;
        }
        const activeHorse = window.state.horses.find(h => h.id === window.state.trainingHorseId);
        if (!activeHorse) return;

        if (activeHorse.training && activeHorse.training.cooldown > 0) {
          alert("冷却期間（リカバー時間）中のため、トレーニングを開始できません。");
          return;
        }
    ...
    case 'TRIGGER_TRAINING_HIT':
      {
        ...
        let desc = 'まずまずのトレーニング効果です！選ばれたステータスが一定期間【+50】強化されます。';

        if (dist <= 6) {
          rating = 'PERFECT';
          amount = 100;
          color = 'text-amber-400 font-extrabold shadow-amber-400 animate-pulse';
          title = '🪐 神聖なる彗星光 (Perfect!)';
          desc = '極限のタイミング！神秘の彗星の力が愛馬に完全に宿り、ステータスが驚異の【+100】強化されます！';
        } else if (dist <= 18) {
          rating = 'GREAT';
          amount = 75;
          color = 'text-emerald-400 font-bold';
          title = '✨ 星霊の調和波動 (Great!)';
          desc = '素晴らしいコントロール！星の加護が高まり、ステータスが【+75】強化されます！';
        }
...
function nextWeek() {
  ...
  const updatedHorses = horses.map(h => {
    ...
    let training = h.training ? { ...h.training } : { activeBoost: null, cooldown: 0 };
    ...
    let nextStats = h.stats;
    if (week === 1) {
      nextStats = calculateGrowth(h);
    }
...
function HorseCard({ h }) {
  const getRankBadgeClass = (rank) => {
    switch (rank) {
      case 'S': return 'bg-gradient-to-r from-amber-400 to-amber-600 text-slate-950 font-black tracking-widest animate-pulse border border-amber-300/30';
      case 'A': return 'bg-gradient-to-r from-orange-500 to-red-500 text-white font-bold border border-orange-400/30';
      case 'B': return 'bg-gradient-to-r from-indigo-500 to-blue-600 text-white font-bold border border-indigo-400/30';
      case 'C': return 'bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-bold border border-blue-400/30';
      default: return 'bg-slate-600 text-slate-200 font-bold border border-slate-500/30';
    }
  };

  const getStrategyLabel = (strat) => {
    switch (strat) {
      case 'escape': return '逃げ';
      case 'pace': return '先行';
      case 'last': return '差し';
      case 'stay': return '追込';
      default: return '自在';
  ...
  const getLineageLabel = (lineage) => {
    switch (lineage) {
      case 'speed': return 'スピード型';
      case 'stamina': return 'スタミナ型';
      case 'guts': return '勝負根性型';
      case 'balance': return 'バランス型';
      default: return '万能型';
    }
  };

  const speedBoost = (h.training?.activeBoost?.stat === 'speed') ? h.training.activeBoost.amount : 0;
  const staminaBoost = (h.training?.activeBoost?.stat === 'stamina') ? h.training.activeBoost.amount : 0;

  const displaySpeed = h.stats.speed + speedBoost;
  const displayStamina = h.stats.stamina + staminaBoost;

  const speedCurrentPct = Math.min(100, Math.max(0, (displaySpeed / 1000) * 100));
  const speedMaxPct = Math.min(100, Math.max(0, ((h.maxStats?.speed || h.stats.speed) / 1000) * 100));
  const staminaCurrentPct = Math.min(100, Math.max(0, (displayStamina / 1000) * 100));
  const staminaMaxPct = Math.min(100, Math.max(0, ((h.maxStats?.stamina || h.stats.stamina) / 1000) * 100));
      ...
      <div class="relative z-10 h-full flex flex-col justify-between gap-5">
         ...
         <div>
           ...
           <div class="grid grid-cols-2 gap-3 bg-slate-950/40 rounded-2xl p-3 border border-white/5">
              <div class="space-y-0.5">
                ...
                <p class="text-xs font-bold text-slate-200">${getStrategyLabel(h.strategy)}</p>
              </div>
              <div class="space-y-0.5 border-l border-white/5 pl-3">
                ...
                <p class="text-xs font-bold text-emerald-400">${h.stats.guts || h.maxStats?.guts || 400}</p>
              </div>
              <div class="space-y-0.5 border-t border-white/5 pt-1.5">
                ...
                <p class="text-xs font-bold text-slate-200">${h.winCount || 0}勝 / ${h.totalRaces || 0}戦</p>
              </div>
              <div class="space-y-0.5 border-t border-l border-white/5 pl-3 pt-1.5">
                ...
                <div class="text-xs font-bold text-amber-500">${h.stats.temperament || h.maxStats?.temperament || 500}</div>
              </div>
           </div>
         </div>
         ...
         <div class="pt-3 border-t border-white/5 flex flex-col gap-2">
           <div class="flex gap-2">
             ${Button({ 
               children: "戦績グラフ・詳細", 
               onClick: () => window.handleAction('OPEN_PERFORMANCE_MODAL', h.id),
               variant: "outline",
               className: "flex-1 text-[11px] py-2 px-1 rounded-xl border-indigo-500/30 text-indigo-400 hover:bg-indigo-500/10 focus:ring-0"
              })}
             ${Button({ 
              children: "レースに出走", 
              onClick: () => window.setState({ screen: 'race_select', selectedHorseId: h.id }),
              className: "flex-1 text-[11px] py-2 px-1 bg-gradient-to-r from-indigo-600/90 to-blue-600/90 hover:from-indigo-600 hover:to-blue-600 border-none rounded-xl"
             })}
           </div>
           <div class="flex gap-2">
              ${!h.isHallOfFameRegistered ? `
                ${Button({
                  children: "🏆 殿堂登録",
                  onClick: () => window.handleAction('REGISTER_HALL_OF_FAME', h.id),
                  variant: "outline",
                  className: "flex-1 text-[10px] py-1.5 rounded-xl border-amber-500/30 text-amber-400 hover:bg-amber-500/10 focus:ring-0 font-extrabold"
                })}
              ` : `
              ...
              `}
              ${Button({
                children: "引退・売却 (¥" + (getHorseSellValue(h) / 10000).toFixed(0) + "万)",
                onClick: () => window.handleAction('OPEN_SELL_MODAL', h.id),
                variant: "outline",
                className: "flex-1 text-[10px] py-1.5 px-2 rounded-xl border-rose-500/15 text-rose-450 hover:text-rose-400 hover:bg-rose-500/15 transition-all focus:ring-0 font-bold"
              })}
            </div>
         </div>
      </div>
        ...
        <div class="pt-8 flex flex-col sm:flex-row justify-center gap-4">
          ${Button({ 
            ...
            onClick: () => {
              ...
              if (hasSave) {
                window.safeConfirm("保存されているデータを上書きして最初から始めますか？", runStart);
              } else {
                runStart();
              }
            },
...
function SceneStable() {
  const { horses, money, year, month, week, stableRankFilter = 'all' } = window.state;

  // Calculate statistics
  const totalCount = horses.length;
  const avgSpeed = totalCount ? Math.round(horses.reduce((sum, h) => sum + h.stats.speed, 0) / totalCount) : 0;
  const avgStamina = totalCount ? Math.round(horses.reduce((sum, h) => sum + h.stats.stamina, 0) / totalCount) : 0;
  ...
  const filteredHorses = horses.filter(h => {
    ...
    return (h.bloodlineRank || 'C').toUpperCase() === stableRankFilter.toUpperCase();
  });
  ...
  const filterTabClass = (active) => active 
      ...
      <header class="p-6 border-b border-indigo-500/20 bg-slate-900/50 backdrop-blur-md sticky top-0 z-10 flex justify-between items-center">
        ...
        <div class="flex gap-2">
          ...
          ${Button({ 
            ...
            variant: "outline",
            className: "text-amber-400 border-amber-500/30 hover:bg-amber-500/10 px-4 flex items-center gap-1 hover:border-amber-400 transition-all font-black"
          })}
          ${Button({ 
            children: "新馬スカウト", 
            onClick: () => window.handleAction('GENERATE_SCOUT_HORSE'),
            variant: "outline",
            className: "text-amber-400 border-amber-500/30 hover:bg-amber-500/10 px-4"
          })}
          ${Button({ 
            children: "配合", 
            onClick: () => {
        ...
        <div class="max-w-7xl mx-auto w-full grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div class="bg-gradient-to-b from-slate-900/40 to-slate-950/60 border ${totalCount >= 8 ? 'border-rose-500/30 shadow-[0_0_15px_rgba(244,63,94,0.15)] animate-pulse' : 'border-slate-900'} rounded-3xl p-5 flex items-center justify-between shadow-[0_4px_20px_rgba(0,0,0,0.15)] hover:border-indigo-500/10 transition-colors">
            <div>
              ...
              <span class="text-3xl font-black italic tracking-tighter ${totalCount >= 8 ? 'text-rose-450' : 'text-white'} font-mono leading-none">${totalCount} <span class="text-xs font-bold text-slate-500">/ 8頭</span></span>
            </div>
      ...
      <div class="max-w-7xl mx-auto w-full px-6 pt-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="space-y-1">
          ...
          <h3 class="text-xl font-black italic text-white tracking-tight uppercase flex items-center gap-2">
            STABLE MANAGEMENT 
            ${horses.length > 0 ? (stableRankFilter !== 'all' ? `
              <span class="text-xs font-extrabold normal-case bg-indigo-950 text-indigo-400 px-3 py-1 rounded-full border border-indigo-900/30">
                ランク <strong>${stableRankFilter.toUpperCase()}</strong> のみ表示 (${filteredHorses.length}頭)
              </span>
            ` : `
              <span class="text-xs font-extrabold normal-case bg-slate-900 text-slate-400 px-3 py-1 rounded-full border border-slate-800">
...
function RenderBreedingProgress(step) {
  return `
    <div class="mb-8 w-fit mx-auto bg-slate-900/80 border border-slate-800/80 rounded-2xl px-6 py-3 flex flex-wrap items-center justify-center gap-4 text-[11px] font-black uppercase tracking-wider font-mono">
      ...
      <span class="text-slate-800 select-none text-xs">➔</span>
...
function SceneBreedingMare() {
  ...
  return `
    <div class="min-h-screen bg-slate-950 text-white flex flex-col p-6 animate-fade-in">
      <!-- Steps Progress -->
      ...
      <header class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex gap-2 p-1 bg-slate-900 border border-slate-800 rounded-2xl w-fit">
          <button class="px-5 py-2 rounded-xl font-bold text-xs transition-all uppercase tracking-wider ${isMarket ? 'bg-pink-600 text-white shadow-lg shadow-pink-500/20' : 'text-slate-400 hover:text-white'}" 
      ...
      ${currentMares.length === 0 ? `
        <div class="flex-1 flex flex-col items-center justify-center p-12 bg-slate-900/40 border border-dashed border-slate-800 rounded-[2rem] text-center space-y-4">
          ...
          <div class="space-y-1">
            ...
            <p class="text-xs text-slate-500 max-w-sm mx-auto">馬房にメス馬（牝）を所有していません。新馬スカウトでメス馬を迎え入れるか、公認繁殖牝馬タブからレンタルして配合を行ってください。</p>
          </div>
          <button class="px-5 py-2 bg-pink-600/20 border border-pink-500/30 text-pink-400 hover:bg-pink-600/30 font-bold rounded-xl text-xs transition-all uppercase" onclick="window.setState({ breedingMareTab: 'market' })">
            公認牝馬カタログを見る
          </button>
        </div>
      ` : `
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-24">
          ${currentMares.map(m => {
            ...
            return `
              <div class="relative bg-slate-900 border border-slate-800/80 rounded-[2rem] p-6 hover:border-pink-500/50 transition-all cursor-pointer group hover:shadow-2xl hover:shadow-pink-500/5" 
                ...
                <div class="mb-4">
                  ...
                  ${isStableDam ? `
                    <div class="text-[10px] text-slate-500 font-bold mt-1">戦績: ${m.winCount || 0}勝 / ${m.totalRaces || 0}戦 | 成長: ${m.growthType === 'early' ? '早熟' : m.growthType === 'late' ? '晩成' : '普通'}</div>
                  ` : `
...
function SceneBreedingStallion() {
  const { selectedMareId, selectedMareSource, breedingStallionTab = 'market', horses } = window.state;
  
  const selectedMare = (selectedMareSource === 'stable'
    ? window.state.horses.find(h => h.id === selectedMareId)
    : window.state.mares.find(m => m.id === selectedMareId)) || { name: "選択中", stats: { speed: 500, stamina: 500 }, bloodlineRank: 'C', lineageId: 'Balance' };
  ...
  return `
    <div class="min-h-screen bg-slate-950 text-white flex flex-col p-6 animate-fade-in pb-32">
      <!-- Steps Progress -->
      ${RenderBreedingProgress(2)}

      <!-- Selected Dam Summary Card -->
      <div class="mb-8 p-5 bg-gradient-to-r from-slate-900 via-pink-950/20 to-slate-900 border border-pink-500/20 rounded-[1.8rem] shadow-xl animate-fade-in">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div class="space-y-1">
            <div class="flex items-center gap-2">
              <span class="text-xs text-pink-400 font-black tracking-widest bg-pink-500/10 px-2.5 py-0.5 rounded border border-pink-500/20">SELECTED DAM (母馬確定済み)</span>
              <span class="text-[10px] text-slate-500 font-bold">➔ 次に以下の牝馬と配合する種牡馬(お父さん)を選んでください。</span>
            </div>
            <div class="flex items-center gap-3 mt-1.5 animate-pulse-subtle">
              ...
              <span class="text-slate-500 text-xs font-mono">| 系統: ${selectedMareLineage}</span>
            </div>
          </div>
      ...
      <!-- Clear glowing separator line with chevron or connector -->
      <div class="relative my-10 flex items-center justify-center">
        <div class="absolute inset-0 flex items-center" aria-hidden="true">
          <div class="w-full border-t border-dashed border-slate-800"></div>
        </div>
        <div class="relative bg-teal-950/20 backdrop-blur-sm px-6 py-2.5 text-[10px] font-black tracking-widest text-pink-400 uppercase flex items-center gap-2 border border-pink-500/30 rounded-full shadow-[0_0_15px_rgba(236,72,153,0.1)]">
          <span class="w-2 h-2 rounded-full bg-pink-500 animate-pulse"></span>
          <span>STEP 1: 牝馬確定 (DAM FIXED)</span>
          <span class="text-slate-600">➔</span>
          <span class="text-sky-450 text-sky-400">STEP 2: 種牡馬選択 (SELECT SIRE)</span>
        </div>
      </div>
      ...
      <header class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <!-- Navigation Tabs -->
        <div class="flex gap-2 p-1 bg-slate-900 border border-slate-800 rounded-2xl w-fit">
          <button class="px-5 py-2 rounded-xl font-bold text-xs transition-all uppercase tracking-wider ${isMarket ? 'bg-sky-600 text-white shadow-lg shadow-sky-500/20' : 'text-slate-400 hover:text-white'}" 
      ...
      ${currentStallions.length === 0 ? `
        <div class="flex-1 flex flex-col items-center justify-center p-12 bg-slate-900/40 border border-dashed border-slate-800 text-center space-y-4 rounded-[2rem]">
          ...
          <div class="space-y-1">
            ...
            <p class="text-xs text-slate-500 max-w-sm mx-auto">馬房にオス馬（牡）を所有していません。新馬スカウトでオス馬を迎え入れるか、公認種牡馬タブからレンタルして配合を行ってください。</p>
          </div>
          <button class="px-5 py-2 bg-sky-600/20 border border-sky-500/30 text-sky-400 hover:bg-sky-600/30 font-bold rounded-xl text-xs transition-all uppercase" onclick="window.setState({ breedingStallionTab: 'market' })">
            種牡馬カタログを見る
          </button>
        </div>
      ` : `
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-24">
          ${currentStallions.map(s => {
            const isStableSire = !isMarket;
            const finalPrice = isStableSire ? 500000 : (window.getStallionPrice ? window.getStallionPrice(s) : (s.price || s.fee || 0));
            const hasDiscount = !isStableSire && window.isMilestoneUnlocked && window.isMilestoneUnlocked('emperor');
            
            return `
              <div class="relative bg-slate-900 border border-slate-800/80 rounded-[2rem] p-6 hover:border-sky-500/50 transition-all cursor-pointer group hover:shadow-2xl hover:shadow-sky-500/5" 
      ...
      `}
      <div class="fixed bottom-0 left-0 right-0 p-6 bg-slate-950/80 backdrop-blur-md border-t border-white/5 mt-8 z-30 flex items-center justify-between">
        ${Button({ children: "← 女性馬を戻り選ぶ", onClick: () => window.setState({ screen: 'breeding_mare' }), variant: "outline" })}
        <span class="text-[10px] text-slate-500 uppercase tracking-widest font-bold">STELLAR RACING BREEDING LAB</span>
      </div>
    </div>
  `;
...
function SceneBreedingConfirm() {
  const { selectedMareId, selectedStallionId, selectedMareSource, selectedStallionSource, useStarBlessing } = window.state;
  
  const mare = (selectedMareSource === 'stable'
    ? window.state.horses.find(h => h.id === selectedMareId)
    : window.state.mares.find(m => m.id === selectedMareId)) || { name: "選択中", stats: { speed: 500, stamina: 500, guts: 500, temperament: 500, health: 500, luck: 500, explosiveness: 500 }, pedigree: { father: '?', mother: '?', grandFathers: ['?','?'], grandMothers: ['?','?'] } };

  const stallion = (selectedStallionSource === 'stable'
    ? window.state.horses.find(h => h.id === selectedStallionId)
    : STALLIONS.find(s => s.id === selectedStallionId)) || { name: "選択中", stats: { speed: 500, stamina: 500, guts: 500, temperament: 500, health: 500, luck: 500, explosiveness: 500 }, pedigree: { father: '?', mother: '?', grandFathers: ['?','?'], grandMothers: ['?','?'] }, explosivePower: 100 };

  const starShards = window.state.starShards || 0;
  const requiredShards = window.getBlessingCost ? window.getBlessingCost() : 5;
  const isStallionDiscounted = selectedStallionSource !== 'stable' && window.isMilestoneUnlocked && window.isMilestoneUnlocked('emperor');
  ...
  else if (maxScore === 5) baseMutationChance = 0.20;
  ...
  else if (sireRank === 'B' && damRank === 'B') baseMutationChance = 0.10;
  ...
  return `
    <div class="min-h-screen bg-slate-950 text-white flex flex-col p-6 animate-fade-in animate-duration-300 pb-32">
      <!-- Steps Progress -->
      ${RenderBreedingProgress(3)}

      <header class="mb-8">
        <div class="flex items-center gap-1.5 mb-1">
          <span class="text-amber-405 text-amber-400 text-sm">🌟</span>
      ...
      <div class="max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-8 pb-12">
        ...
        <div class="lg:col-span-7 space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 font-sans">
            <!-- Mother Card -->
            <div class="bg-slate-900 border border-slate-800/60 rounded-[2rem] p-6 relative overflow-hidden">
            ...
            <div class="bg-slate-900 border border-slate-800/60 rounded-[2rem] p-6 relative overflow-hidden">
              ...
              <div class="mt-6 space-y-3">
                <div>
                  ...
                  <div class="h-1.5 bg-slate-950 rounded-full overflow-hidden"><div class="h-full bg-indigo-505" style="width: ${(stallion.stats.speed/1000)*100}%"></div></div>
                </div>
                <div>
          ...
          <div class="bg-slate-900/60 border border-slate-850 p-6 rounded-[2rem] space-y-4 font-sans">
            <h4 class="text-xs font-black tracking-wider uppercase text-slate-400 flex items-center gap-1.5">
          ...
          <div class="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 space-y-6 font-sans">
            ...
            <div class="space-y-4">
              ...
              <div class="p-4 rounded-3xl bg-slate-950/60 border border-white/5 space-y-2.5">
                ...
                <div class="flex items-center justify-between">
                  <span class="text-xs text-slate-400 font-bold font-sans">ニックス発生 (Special Nick):</span>
                ...
                <div class="flex items-center justify-between">
                  <span class="text-xs text-slate-400 font-bold font-sans">爆発力 (Explosiveness):</span>
                  <span class="text-xs font-mono font-bold text-amber-400">${stallion.explosivePower || 100} EP</span>
                </div>
                <div class="flex items-center justify-between">
                  <span class="text-xs text-slate-400 font-bold font-sans">近親交配 (Inbreeding Factor):</span>
                  <span class="text-xs font-mono font-black ${inbreedingChars.length > 0 ? 'text-red-400' : 'text-slate-400'}">
              ...
              <div class="p-4 rounded-3xl border border-dashed border-amber-500/30 bg-amber-950/10 space-y-3">
                ...
                <p class="text-[9px] text-slate-400 leading-relaxed font-sans font-sans">
                  ...
                  <span class="text-amber-300 font-semibold block mt-1">【効果】突然変異率がさらに <span class="text-amber-400 text-xs font-black">+15%</span> 底上げされ、2つの金特性と超限界パラメーターが確定します。</span>
                </p>
                <div class="flex pt-1 font-sans">
                  ${Button({
                    children: useStarBlessing ? "★ 加護を適用中" : `星片を ${requiredShards}個 消費して加護を適用する`,
                    onClick: () => {
                      if (starShards < requiredShards) {
                        alert("星片が足りません。テーマ別イベントで上位入賞して星片を集めてください。");
                        return;
                      }
            ...
            <div class="pt-4 border-t border-slate-800">
              <div class="flex justify-between items-center mb-5 font-sans">
                <span class="text-xs font-bold text-slate-400 uppercase font-sans">BREEDING FEE (配合費用/種付費用)</span>
              ...
              <div class="flex gap-4">
                ${Button({ 
                  ...
                  className: "flex-1 py-4 text-base font-black bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 shadow-xl shadow-indigo-600/15 border-none rounded-2xl" 
                })}
                ${Button({ 
                  children: "キャンセル", 
                  onClick: () => window.setState({ screen: 'breeding_stallion' }), 
                  variant: "outline", 
                  className: "px-6 border-slate-800 text-slate-400 hover:text-white rounded-2xl text-[11px] py-4" 
                })}
              </div>
            </div>
          </div>
...
export function evaluateWeatherSuitability(horse, weather) {
  if (!horse) {
    return { rating: 'B', color: 'bg-slate-850 text-slate-400 border border-white/5', ratingText: '良馬場適性' };
  }
  ...
  if (weather === 'Sunny') {
    if (lineage === 'speed' || speed > 700) {
      return { rating: 'S', color: 'bg-gradient-to-r from-amber-400 to-amber-600 text-slate-950 font-black', ratingText: '良馬場・快晴適性抜群' };
    }
    return { rating: 'A', color: 'bg-emerald-500 text-slate-950 font-bold', ratingText: '良馬場適性あり' };
  } else if (weather === 'Rainy') {
    if (lineage === 'guts' || stamina > 650) {
      return { rating: 'A', color: 'bg-emerald-500 text-slate-950 font-bold', ratingText: '重馬場適性あり' };
    }
    return { rating: 'B', color: 'bg-slate-800 text-slate-350 border border-slate-705/30', ratingText: '雨天標準' };
  } else {
    if (lineage === 'stamina' || stamina > 750) {
      return { rating: 'S', color: 'bg-gradient-to-r from-indigo-505 to-blue-600 text-white font-black', ratingText: '不良馬場適性抜群' };
    }
    if (stamina > 550) {
      return { rating: 'B', color: 'bg-slate-800 text-slate-355 border border-slate-705/30', ratingText: '不良馬場適性あり' };
    }
    return { rating: 'C', color: 'bg-slate-950/60 text-slate-500 border border-white/5', ratingText: '重・泥濘苦手' };
  }
}
...
export function getRaceForecast(r) {
  ...
  if (seed.includes('1') || seed.includes('5')) {
    return { weather: 'Sunny', name: '良馬場・快晴', icon: '☀️', probability: 90 };
  } else if (seed.includes('2')) {
    return { weather: 'Rainy', name: '重馬場・小雨', icon: '🌧️', probability: 80 };
  } else {
    return { weather: 'Muddy', name: '不良・豪雨', icon: '🌊', probability: 75 };
  }
}

export function calculateRaceSuitability(horse, race, forecast) {
  if (!horse) return { rating: '👌 標準 (B)', color: 'bg-slate-800 text-slate-300' };
  ...
  if (score >= 4) return { rating: '🌟 特上 (EX)', color: 'bg-gradient-to-r from-amber-400 to-amber-600 text-slate-950' };
  if (score === 3) return { rating: '👍 優位 (A)', color: 'bg-emerald-500 text-slate-950' };
  if (score === 2) return { rating: '👌 標準 (B)', color: 'bg-slate-800 text-slate-300' };
  return { rating: '⚠️ 懸念 (C)', color: 'bg-rose-500/10 text-rose-400 border border-rose-500/20' };
}

function SceneRaceSelect() {
  const horse = window.state.horses.find(h => h.id === window.state.selectedHorseId) || { name: "所有馬", stats: { speed: 500, stamina: 500 } };
  ...
  return `
    <div class="min-h-screen bg-slate-950 text-white flex flex-col p-6">
      ...
      <div class="flex gap-4 border-b border-white/5 pb-4 mb-8">
        <button class="px-6 py-2.5 rounded-2xl text-xs font-black transition-all ${
          currentTab === 'regular' 
            ...
            <div class="flex-1 space-y-2.5">
              <div class="flex flex-wrap items-center gap-2.5">
                <span class="px-2.5 py-1 bg-slate-800 rounded-lg text-[10px] uppercase font-black tracking-tight ${
                  currentTab === 'themed' ? 'text-amber-400 border border-amber-500/10' : 'text-indigo-400'
                }">${r.grade}</span>
                <span class="text-xs text-slate-400 font-mono font-bold">${r.distance}m</span>
              ...
              <p class="text-xs text-slate-400 leading-relaxed font-sans">${r.desc}</p>
            </div>
...
function SceneRaceResult() {
  ...
  if (window.state.screen === 'race_result' && window.state.animatedResults === false) {
    ...
    setTimeout(() => {
      ...
      const t1 = setTimeout(() => {
        if (window.state.screen === 'race_result' && window.state.animatedResults) {
  ...
  const commentaryList = generateFinishLineCommentary(playerResult, sortedResults[0]);
  const currentCommentStep = Math.min(3, Math.max(0, window.state.replayCommentStep || 0));
  const currentComment = commentaryList[currentCommentStep] || "最後の勝負がここからはじまります！解説・実況にご注目ください。";
  ...
  if (playerResult.position > 5 && !displayHorses.some(h => h.horseId === playerResult.horseId)) {
      ...
      <div class="max-w-3xl w-full space-y-8">
        <header class="text-center space-y-2">
          ...
          <div class="flex items-center justify-center gap-1.5 flex-wrap justify-center mt-1">
            <span class="text-xs text-slate-500 font-bold">${window.state.currentRace?.name || ''} (${window.state.currentRace?.distance || 0}m)</span>
        ...
        <div class="space-y-4 relative p-8 bg-slate-900/60 border border-slate-800 rounded-[3rem] shadow-[0_8px_32px_rgba(0,0,0,0.3)] select-none">
          <div class="flex items-center justify-between mb-4 border-b border-white/5 pb-3">
            <div class="flex items-center gap-2">
              ...
              <h3 class="text-xs font-black uppercase tracking-wider text-slate-300">ゴール直前リプレイ (Finish Line Replay)</h3>
            </div>
          ...
          <div class="relative w-full h-[410px] bg-slate-950/90 rounded-2xl border border-white/5 p-4 flex flex-col justify-between overflow-hidden shadow-inner font-sans">
            ...
            <div class="flex-1 flex flex-col justify-start gap-2.5 pb-24 overflow-y-auto z-10">
              ${displayHorses.map((r, idx) => {
                const isPlayer = r.horseId === window.state.selectedHorseId;
                // Starting line is 5%, Goal is up to 88% depending on performance margin
                const finalPct = window.state.animatedResults ? Math.max(8, 85 - (r.position - 1) * 7.5) : 5;
                const rankColor = r.position === 1 ? 'bg-amber-500 text-slate-950 font-black' : 
                                  r.position === 2 ? 'bg-slate-350 text-slate-950' : 
                                  r.position === 3 ? 'bg-amber-700 text-white' : 'bg-slate-800 text-slate-400';
            ...
            <div class="absolute bottom-4 left-4 right-4 bg-slate-950/95 border border-indigo-500/30 rounded-2xl p-3.5 flex gap-3.5 items-center backdrop-blur-md shadow-2xl z-20 min-h-[72px] transition-all duration-300">
              ...
              <div class="flex-1 text-left">
                ...
                <p class="text-xs text-slate-100 font-bold leading-relaxed italic transition-all duration-300">
                  ${currentComment}
                </p>
              </div>
            </div>
        ...
        <div class="flex justify-center">
           ${Button({ 
             children: "牧場に戻る (Return to Stable)", 
             onClick: () => window.setState({ screen: 'stable', currentRace: null, raceResult: null, raceStep: 0, raceRewards: null }),
             className: "px-12 py-4 text-lg bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 border-none rounded-2xl"
           })}
        </div>
      </div>
    </div>
        ...
        <div class="space-y-4 pt-4 border-t border-white/5 text-left text-white">
          ...
          <div class="pt-4 border-t border-white/5 flex gap-3">
            ...
            ${Button({
              children: "データを完全初期化",
              onClick: () => {
                window.safeConfirm("本当に保存されているすべてのゲームデータをリセットしますか？この操作は取り消せません。", () => {
                  localStorage.removeItem(SAVE_KEY);
                  window.setState({
                    screen: 'title',
                    horses: [],
                  ...
                  window.safeAlert("すべてのセーブデータを完全に初期化しました。");
                });
              },
              variant: "danger",
...
function RenderScoutModal() {
  ...
  const getRankBadgeColor = (rank) => {
    switch (rank) {
      case 'S': return 'bg-amber-500 text-slate-950 border-amber-400 font-bold';
      case 'A': return 'bg-orange-500 text-white border-orange-400';
      case 'B': return 'bg-indigo-500 text-white border-indigo-400';
      case 'C': return 'bg-blue-500 text-white border-blue-400';
    ...
    <div class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-slate-900 border border-slate-800 rounded-[2.5rem] max-w-lg w-full p-8 space-y-6 relative overflow-hidden shadow-[0_0_50px_rgba(99,102,241,0.15)]">
        ...
        <div class="border border-white/5 bg-slate-950 rounded-2xl p-5 flex items-center gap-5">
          <div class="w-16 h-16 rounded-2xl flex items-center justify-center bg-slate-900 border border-white/10 relative shadow-[inset_0_0_15px_rgba(255,255,255,0.05)]">
            <div class="w-2/3 h-2/3 rounded-full shadow-[0_0_15px_rgba(255,255,255,0.3)]" style="background-color: ${h.color}"></div>
          </div>
          <div class="flex-1 space-y-1">
            <div class="flex items-center gap-2 flex-wrap">
              <span class="text-xl font-black text-white italic tracking-tight uppercase">${h.name}</span>
              <span class="px-2.5 py-0.5 rounded-full text-xs font-black tracking-wider border ${getRankBadgeColor(h.bloodlineRank)} shadow-[0_0_12px_rgba(245,158,11,0.15)]">
        ...
        <div class="space-y-4">
          <div class="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-white/5 pb-1">能力ポテンシャル (POTENTIAL STATS)</div>
        ...
        <div class="pt-4 border-t border-white/5 flex flex-col gap-3">
          ...
          <div class="flex gap-3 mt-1.5">
            ...
            ${Button({
              children: canAfford ? "スカウトする (¥200万)" : "資金が足りません",
          ...
          <div class="flex justify-center">
            ${Button({
              ...
              onClick: () => window.handleAction('GENERATE_SCOUT_HORSE'),
              variant: "outline",
              className: "text-[10px] text-slate-400 border-white/5 hover:border-slate-700/50 py-1.5 px-4 rounded-full mt-1.5"
            })}
          </div>
        </div>
      </div>
    ...
    <div class="fixed inset-0 bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div class="bg-slate-900 border border-slate-800 rounded-[2.5rem] max-w-md w-full p-8 space-y-6 relative overflow-hidden shadow-[0_0_50px_rgba(244,63,94,0.15)]">
        ...
        <div class="p-5 bg-gradient-to-r from-rose-950/20 to-slate-950/50 rounded-2xl border border-rose-500/20 text-center space-y-2">
          <p class="text-xs text-slate-400 font-bold">受け取る引退保証・売却資金</p>
          <p class="text-3xl font-black italic tracking-tighter text-rose-400 font-mono">${formatMoney(sellPrice)}</p>
        </div>

        <p class="text-xs text-slate-400 leading-relaxed font-bold bg-slate-950/30 p-4 border border-slate-850 rounded-xl text-center">
        ...
        <div class="flex gap-3">
          ${Button({ 
            children: "キャンセル", 
            onClick: () => window.setState({ showSellModal: false, sellingHorseId: null }),
            variant: "outline",
          ...
          ${Button({ 
            ...
            variant: "primary",
            className: "flex-1 rounded-xl bg-gradient-to-r from-rose-600 to-rose-500 border-none hover:from-rose-500 hover:to-rose-450 font-bold"
          })}
        </div>
      </div>
    </div>
...
function RenderTrainingModal() {
  const { showTrainingModal, trainingHorseId, horses, starShards, trainingStage, trainingType, trainingResult, gaugePosition } = window.state;
      ...
      <div class="space-y-5 animate-fade-in">
        ...
        <div class="p-3 bg-slate-950/60 rounded-2xl border border-white/5 flex items-center gap-3">
          ...
          <div class="min-w-0 flex-1">
            <div class="text-xs text-slate-500 font-bold uppercase">特訓対象</div>
            <div class="text-md font-black italic tracking-tight uppercase text-white truncate">${h.name}</div>
          </div>
          ${isCooldown ? `
            <div class="text-right px-2.5 py-1 bg-rose-500/10 border border-rose-500/20 text-rose-450 rounded-lg text-xs font-bold shrink-0">
              回復待ち: ${h.training.cooldown}週
            </div>
          ` : `
            <div class="text-right px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-lg text-xs font-bold shrink-0">
              特訓可能
            </div>
          `}
        </div>
        ...
        <div class="p-4 rounded-2xl border border-sky-500/20 bg-slate-950/40 text-center space-y-1">
          ...
          <div class="flex items-center justify-center gap-1.5">
            ...
            <span class="text-slate-400 text-xs">個所持</span>
          </div>
        ...
        <!-- Training Selections -->
        <div class="space-y-3">
          <p class="text-xs text-slate-400 font-bold uppercase tracking-widest">特訓メニューを選択</p>
        ...
        <div class="space-y-2">
          ${isCooldown ? `
            <div class="text-center text-xs text-rose-450 font-bold bg-rose-500/10 py-2.5 rounded-xl border border-rose-500/20">
              🚨 冷却回復中のため特訓できません（リカバー完了までお待ちください）
            </div>
          ` : starShards < 3 ? `
            <div class="text-center text-xs text-amber-500 font-bold bg-amber-500/10 py-2.5 rounded-xl border border-amber-500/20">
              ⚠️ 星片が足りません（彗星特訓には星片3個が必要です）
            </div>
          ` : !trainingType ? `
            <div class="text-center text-xs text-indigo-400 font-bold bg-indigo-500/10 py-2.5 rounded-xl border border-indigo-500/20">
              👉 特訓メニューを上から選択してください
            </div>
          ` : `
          ...
          <div class="flex gap-3 pt-1">
            ...
            ${Button({ 
              children: "特訓ミニゲーム開始！", 
              onClick: () => window.handleAction('START_TRAINING_GAME', trainingType),
              variant: "primary",
              className: "flex-1 rounded-xl bg-gradient-to-r from-indigo-500 to-sky-500 border-none hover:from-indigo-600 hover:to-sky-600 font-bold shadow-[0_4px_15px_rgba(99,102,241,0.25)]",
    ...
    modalContent = `
      <div class="space-y-6 animate-fade-in text-center">
        <div>
          ...
          <p class="text-xs text-slate-400">ポインターが中心のゴールド（50%付近）の時にタップしてください！</p>
        </div>
        ...
        <div class="pt-2">
          ${Button({ 
            ...
            variant: "primary",
            className: "w-full py-4 text-md bg-gradient-to-r from-violet-600 via-indigo-600 to-blue-600 hover:from-violet-500 hover:to-blue-500 border-none rounded-2xl font-black shadow-[0_0_20px_rgba(99,102,241,0.4)] animate-pulse"
          })}
        </div>
      </div>
    `;
    ...
    modalContent = `
      <div class="space-y-6 animate-fade-in text-center pb-2">
        ...
        <div class="space-y-1.5">
          ...
          <p class="text-xs text-slate-400 max-w-sm mx-auto leading-relaxed pt-1.5">${desc}</p>
        </div>
        ...
        <div class="p-5 bg-gradient-to-r from-slate-950 via-slate-950/80 to-slate-950 rounded-3xl border border-white/5 space-y-3">
          ...
          <div class="h-[1px] bg-white/5"></div>
          <div class="flex justify-between items-center px-4">
            <span class="text-xs text-slate-500 font-bold">回復リカバー時間</span>
            <span class="font-bold text-xs text-rose-450 font-mono">特訓終了後 4週間</span>
          </div>
        </div>
        ...
        <div class="pt-2">
          ${Button({ 
            ...
            onClick: () => window.handleAction('CLOSE_TRAINING_MODAL'),
            variant: "primary",
            className: "w-full rounded-xl bg-indigo-600 hover:bg-indigo-500 border-none font-bold"
          })}
        </div>
      </div>
    `;
    ...
    <div class="fixed inset-0 bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in" id="game-dialog-modal" style="z-index: 10000;">
      <div class="bg-slate-900 border border-slate-800 rounded-[2.5rem] max-w-md w-full p-8 space-y-6 relative overflow-hidden shadow-[0_0_70px_rgba(244,63,94,0.25)]">
        ...
        <div class="flex gap-4 pt-2 relative">
          ${isConfirm ? `
            <button class="flex-1 px-5 py-3 rounded-2xl bg-slate-800 hover:bg-slate-705 text-slate-300 font-bold text-xs transition-colors uppercase tracking-wider cursor-pointer active:scale-95" 
                    onclick="if (window.state.dialog && window.state.dialog.onCancel) window.state.dialog.onCancel();">
              キャンセル (Cancel)
            </button>
            <button class="flex-1 px-5 py-3 rounded-2xl bg-gradient-to-r from-pink-600 to-rose-600 hover:from-pink-500 hover:to-rose-500 text-white font-black text-xs shadow-lg shadow-pink-500/20 transition-all uppercase tracking-widest cursor-pointer active:scale-95" 
                    onclick="if (window.state.dialog && window.state.dialog.onConfirm) window.state.dialog.onConfirm();">
              確認する (Confirm)
            </button>
          ` : `
            <button class="w-full px-5 py-3 rounded-2xl bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-505 hover:to-indigo-605 text-white font-black text-xs shadow-lg shadow-indigo-500/20 transition-all uppercase tracking-widest cursor-pointer active:scale-95" 
                    onclick="if (window.state.dialog && window.state.dialog.onConfirm) window.state.dialog.onConfirm();">
              OK 閉じる
            </button>
          `}
...
function render() {
  ...
  switch (window.state.screen) {
    ...
    case 'hall_of_fame': content = window.SceneHallOfFame ? window.SceneHallOfFame() : ''; break;
    default: content = SceneTitle();
  }

  app.innerHTML = content + RenderSaveModal() + RenderScoutModal() + RenderSellModal() + RenderTrainingModal() + RenderGameDialogModal() + '<div id="performance-chart-modal-root"></div>';
